...
def login(db = Depends(get_db) , form_data: OAuth2PasswordRequestForm = Depends()):
    creator = get_creator(db, form_data.username)

    if not creator or not verify_password(form_data.password, creator.password_hash):
    ...
    refresh_token = create_refresh_token(data={"sub": creator.email})
